...
//   body: { name: string; description?: string } // Project details
// ) => {
// const project = new ProjectModel({// Add emoji if provided
...
export const getProjectByIdAndWorkspaceIdService = async (
  ...
  const project = await ProjectModel.findOne({
    workspace: workspaceId, // Match workspace ID
    _id: projectId, // Match project ID
  }).select('_id name description createdBy'); // Select specific fields
...
export const updateProjectByIdAndWorkspaceIdService = async (
  workspaceId: string, // ID of the workspace
  projectId: string, // ID of the project
  body: { name: string; description?: string } // Updated project details
) => {
  const { name, description } = body;
